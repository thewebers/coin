...
{-
TODOs
- merge chains generated by multiple miner threads
- actually sign transactions with sender private key
- transaction binning to lower fan-in
- speed up mining by saving penultimate hash ctx
- verify (as the miner) the user can't spend other users' transactions
- verify (as the user) that transactions we
- migrate tests to use STM (so shit compiles)
-}
